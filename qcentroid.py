...
def sqa_move(plan, cm, layout, vbm, progress):
    ...
    if r < (1-vessel_pct)*0.6:
        return swap_random(plan, cm, layout)
    elif r < (1-vessel_pct):
        return relocate_random(plan, cm, layout)
    elif r < (1-vessel_pct) + vessel_pct*0.55:
        return swap_vessel(plan, cm, layout, vbm)
    else:
        return relocate_vessel(plan, cm, layout, vbm)
